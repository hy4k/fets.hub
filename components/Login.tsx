...
export const Login: React.FC<LoginProps> = ({ onLogin, onCreateUser }) => {
  ...
  // Create enough items to fill a large screen
  const bgImages = [...INITIAL_PROJECTS, ...INITIAL_PROJECTS, ...INITIAL_PROJECTS, ...INITIAL_PROJECTS].slice(0, 24);

  return (
    <div className="min-h-screen bg-black relative overflow-hidden flex items-center justify-center">
      
      {/* Dynamic Background Collage */}
      <div className="absolute inset-0 grid grid-cols-3 md:grid-cols-6 gap-1 opacity-40 grayscale scale-105">
        {bgImages.map((project, idx) => (
          <div key={idx} className="relative w-full h-48 overflow-hidden">
             <img 
               src={project.imageUrl} 
               alt="" 
               className="w-full h-full object-cover opacity-60 hover:opacity-100 hover:scale-110 transition-all duration-1000" 
             />
          </div>
        ))}
      </div>
        ...
        <div className="bg-black/60 backdrop-blur-xl border border-white/10 rounded-lg p-8 shadow-2xl">
          ...
          <div className="mt-6 flex items-center justify-between text-xs text-gray-400">
            ...
            <button type="button" onClick={() => setIsSignUp(!isSignUp)} className="text-white hover:underline">
               {isSignUp ? 'Already a member? Sign in.' : 'New to FETS HUB? Sign up now.'}
            </button>
          </div>

        </div>

        {/* Quick Demo Access - Replaces the old list */}
        {!isSignUp && (
           <div className="mt-8">
