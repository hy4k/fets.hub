...
import { X, Play, Plus, Check, ThumbsUp, Github, Code, FileText, Terminal, Edit, Trash2, Folder, Copy, Tag, User as UserIcon, Share2 } from 'lucide-react';
...
export const ProjectDetailView: React.FC<ProjectDetailViewProps> = ({ 
  ...
  const handleShareProject = () => {
    // Copies the current URL (assuming the user wants to share the dashboard)
    // or constructs a message if no routing is present
  ...
  const handleOpen = () => {
    if (project.websiteUrl) {
      // This handles both https:// links and custom app protocols (e.g. vscode://, steam://)
      window.open(project.websiteUrl, '_blank');
    } else {
      alert(`No launch configuration found for "${project.name}".\n\nPlease add a Website URL or App URI (e.g., myapp://run) in the project settings.`);
    }
  };
